def big(tim):
    ...
    def iLV(relative_abundance, minutes, num_run, n0_est, num_iteration, output_dir, absolute_abundance: None):
        ...
        fig1.savefig(f"{output_dir}/abundance_over_time.png") #removed _{min_distance:.2f}_{method_used}
        ...
        fig2.savefig(f"{output_dir}/plate_{minutes}.png") #emma
    ...
    if True: #__name__ == "__main__":
        import sys
        relative_abundance = "data/cheese_relative.csv" #if more dataseta, maybe a menu to choose
        minutes = int(tim) #int(sys.argv[2]) #from html input !!!
        num_run = int(5*minutes) #int(sys.argv[3]) #since they take ~ 20 seconds and buffer is better
        n0_est = float(0.01) #float(sys.argv[4]) #took this bc of optimum
        num_iteration = int(30)# int(sys.argv[5]) #arbitrary
        output_dir = "outputs" #str(sys.argv[6])
        if len(sys.argv) > 7:
